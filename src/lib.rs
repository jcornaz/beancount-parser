...
impl<D: Decimal> ReadFilesIter<D> {
    ...
    fn load_next_file(&mut self) -> Result<bool, ReadFileError> {
        while let Some(path) = self.pending.pop() {
            ...
            File::open(&path)?.read_to_string(&mut self.str_buffer)?;

            let entries = parse_iter::<D>(&self.str_buffer).collect::<Result<Vec<_>, _>>()?;
...
impl<D: Decimal> Iterator for ReadFilesIter<D> {
    type Item = Result<Entry<D>, ReadFileError>;
